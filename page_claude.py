...
class MasterGuardian_Smart_Claude:
    def __init__(self):
        ...
        # [보존] 제외 키워드 리스트 (트럼프, WHO, 사설 등 포함)
        self.exclude_ad = [
            "변호사", "법무법인", "법률사무소", "선임", "상담문의", "무료상담", "승소",
            "카톡문의", "직통전화", "전화번호", "실력있는", "알아보고있다면",
            "홍보", "마케팅", "협찬", "보도자료", "캠페인", "분양", "입주", "청약", "특가", "할인", 
            "세일", "프로모션", "무료증정", "체험단", "쇼룸", "총정리", "알아보자", "방법은", "이유는", 
            "성공사례", "무료진단", "인사동정", "협약", "mou", "체결", "출범", "맞손", "시구", "방문",
            "칼럼", "사설", "기고", "기자시각", "독자투고", "시론", "가판", "운세", "부고", "인사", 
            "ceo스토리", "기업인사", "인물열전", "who is", "who", "WHO", "주년", "탄생", "e종목",
            "사업 시동", "실증 추진", "전략 공유", "등급 허가", "Q&A", "합동 교육", "교육 실시",
            "공항날씨", "오늘날씨", "제주날씨", "날씨예보", "도약시킬 것", "패트롤", "월드비전",
            "레바논 분쟁", "적십자", "만원 기부", "중국 자동차", "한국 자동차", "피스타치오 가격",
            "제 2의 김창민", "사법 민낯", "계열사 누락", "금융 HOT 뉴스", "성금", "브리프", "이야기", 
            "브리핑", "인터뷰", "interview", "인물", "論하다", "기업家", "겜덕", "이모저모", "세미나", 
            "강원소방", "지휘관 회의", "트럼프", "라운지", "포커스", "소식", "재계는", "레이더", "사사건건", 
            "와글와글", "오프닝"
        ]
        self.exclude_entertainment = [
            "방영", "예능", "본방", "시청률", "컴백", "데뷔", "무대", "아이돌", "솔로", "앨범", 
            "차트", "박스오피스", "제작보고회", "회상했다", "회고", "추억", "성공 비결",
            "팬미팅", "굿즈", "직캠", "fancam", "열애", "결별", "이별", "교제", "프로야구", "야구", 
            "관중", "피치클록", "홈런"
        ]
...
def run_claude_collector():
    ...
    with c4:
        if st.button("체크", use_container_width=True):
        ...
        with st.status("📡 수집 중...", expanded=True) as status:
            ...
            if os.path.exists('언론키워드셋.txt'):
                ...
                for idx, kw in enumerate(keywords):
                    status.update(label=f"🔎 [{idx+1}/{len(keywords)}] '{kw}'")
                    if st.session_state.get('stop_flag', False): break
    ...
    if st.session_state.claude_pool:
        ...
        # [수정] 슈 님 요청: 칸 폭 최소화 및 시간/키워드 분리
        edited = st.data_editor(
            df,
            column_config={
                "수집시간": st.column_config.TextColumn("시간", width=85), # 시:분까지만 나오므로 최소화
                "수집키워드": st.column_config.TextColumn("키워드", width=100),
                "기사제목": st.column_config.TextColumn("분석 대상 헤드라인"), # 제목은 유동적으로 확보
                "링크": st.column_config.LinkColumn(" ", display_text="🔗", width=40), # 아이콘만 보이게 최소화
                "선택": st.column_config.CheckboxColumn(" ", width=40), # 체크박스 최소화
            },
        ...
        if not sel.empty:
            ...
            with d1:
                ...
                st.download_button("📄 제목(.txt)", titles_only.encode('utf-8'), "Titles.txt", use_container_width=True)
            with d2:
                out = io.BytesIO()
                sel.drop(columns=['선택']).to_excel(out, index=False, engine='openpyxl')
                st.download_button("📊 엑셀 저장", out.getvalue(), "Report.xlsx", use_container_width=True)
            with d3:
                engine_temp = MasterGuardian_Smart_Claude()
                full_prompt = engine_temp.make_claude_prompt("\n".join([f"- {t}" for t in sel['기사제목']]))
                st.download_button("📝 프롬프트 저장", full_prompt.encode('utf-8'), "Claude.txt", use_container_width=True)
